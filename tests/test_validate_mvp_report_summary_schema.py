...
def test_validate_summary_payload_accepts_supported_report_type(monkeypatch) -> None:
    def fake_load_schema(name: str) -> dict[str, object]:
        assert name == "mvp_first_analytics_summary.schema.json"
        return {
            "type": "object",
            "required": ["report_type"],
            "properties": {"report_type": {"const": "mvp_first_analytics"}},
        }

    monkeypatch.setattr(validate_summary_schema_module, "load_schema", fake_load_schema)

    schema_path = validate_summary_payload({"report_type": "mvp_first_analytics"})
