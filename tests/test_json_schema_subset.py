...
def test_assert_valid_schema_accepts_unexpected_key_when_additional_properties_omitted() -> None:
    assert_valid_schema(
...
def test_assert_valid_schema_accepts_declared_keys_when_additional_properties_false() -> None:
    assert_valid_schema(
        {"name": "alpha"},
        {
            "type": "object",
...
def test_assert_valid_schema_accepts_declared_property_value() -> None:
    assert_valid_schema(
        {"name": "alpha"},
        {
            "type": "object",
...
def test_assert_valid_schema_rejects_nullable_union_type_mismatch() -> None:
    try:
        assert_valid_schema(1, {"type": ["string", "null"]})
    except ValueError as exc:
        assert "expected type ['string', 'null']" in str(exc)
    else:
...
def test_assert_valid_schema_accepts_nullable_date_union_with_string_value() -> None:
