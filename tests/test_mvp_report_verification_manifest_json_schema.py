...
def test_mvp_report_verification_manifest_schema_rejects_missing_required_nested_report_key(tmp_path: Path) -> None:
    first_dir = tmp_path / "first"
    live_dir = tmp_path / "live"
    first_dir.mkdir()
    live_dir.mkdir()
    ...
    manifest = build_manifest(first_dir, live_dir)
    del manifest["reports"]["first_analytics"]["summary_path"]
    schema = _load_schema("mvp_report_verification_manifest.schema.json")
    ...
    try:
        ...
        raise AssertionError("expected verification manifest schema to reject a missing required nested report key")
