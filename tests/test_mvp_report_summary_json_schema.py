...
def test_mvp_first_analytics_summary_schema_rejects_missing_required_top_level_key(tmp_path: Path) -> None:
    _write_json(tmp_path / "requests.json", {"generated_at": "2030-01-01T00:00:00+00:00", "base_url": "http://127.0.0.1:8010", "requests": []})
    _write_json(
        tmp_path / "production_order_direct.json",
        {
...
def test_mvp_first_analytics_summary_schema_rejects_missing_required_request_field(tmp_path: Path) -> None:
    _write_json(
        tmp_path / "requests.json",
        {
            "generated_at": "2030-01-01T00:00:00+00:00",
            ...
            "requests": [
                {"name": "planning-core-health", "method": "GET", "url": "http://127.0.0.1:8010/health", "body": None},
                {"name": "production-order-direct", "method": "POST", "url": "http://127.0.0.1:8010/proposal", "body": {"article_id": 1}},
            ],
        },
    )
    _write_json(
    ...
    summary = build_first_analytics_summary(tmp_path)
    del summary["production_order_direct"]["status"]
    schema = _load_schema("mvp_first_analytics_summary.schema.json")
    ...
    try:
        ...
        raise AssertionError("expected first analytics summary schema to reject nested data missing status")
