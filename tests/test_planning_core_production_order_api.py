...
def test_production_order_proposal_from_wb_endpoint(client, db_session):
    ...
    assert alpha_proxy["economic_source"]["average_realized_price_main"] == LAYER_PROXY_VALUE_SOURCE
    assert alpha_proxy["economic_source"]["average_realized_price_assorti"] == LAYER_PROXY_VALUE_SOURCE
    assert alpha_proxy["economic_inputs"]["average_realized_price_main"] == 1.8
    assert alpha_proxy["economic_inputs"]["average_realized_price_assorti"] == 1.65
    economics_trust = body["explanation"]["meta"]["economics_trust"]
...
def test_production_order_proposal_from_wb_reports_empty_token_commission_metadata(client, db_session):
    seeded = _seed_article_bundle_base(db_session)

    commission_account = WbIntegrationAccount(
        name="WB Empty Token Commission",
        supplier_id=None,
        api_token="   ",
        is_active=True,
    )
    ...
    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-COMMISSION-EMPTY-TOKEN",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
    ...
    commission_meta = full_from_wb["economic_observed_commission"]
    assert commission_meta["source"] == FROM_WB_TARIFFS_COMMISSION_SOURCE
    assert commission_meta["status"] == "unavailable"
    assert commission_meta["reason"] == "empty_api_token"
    assert commission_meta["account_id"] == commission_account.id
    assert commission_meta["fetched_rows"] == 0
...
def test_production_order_proposal_from_wb_reports_unauthorized_commission_metadata(
    client,
    db_session,
    monkeypatch,
):
    ...
    commission_account = WbIntegrationAccount(
        name="WB Unauthorized Commission",
        supplier_id=None,
        api_token="token-unauthorized",
        is_active=True,
    )
    db_session.add(commission_account)
    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-COMMISSION-UNAUTHORIZED",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
    ...
    assert commission_meta["source"] == FROM_WB_TARIFFS_COMMISSION_SOURCE
    assert commission_meta["status"] == "unavailable"
    assert commission_meta["reason"] == "unauthorized"
    assert commission_meta["account_id"] == commission_account.id
    assert commission_meta["fetched_rows"] == 0
    assert commission_meta["subjects_with_commission"] == 0
    assert commission_meta["commission_percent"] == {"main": None, "assorti": None}
...
def test_production_order_proposal_from_wb_reports_invalid_json_commission_metadata(
    client,
    db_session,
    monkeypatch,
):
    ...
    commission_account = WbIntegrationAccount(
        name="WB Invalid JSON Commission",
        supplier_id=None,
        api_token="token-invalid-json",
        is_active=True,
    )
    db_session.add(commission_account)
    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-COMMISSION-INVALID-JSON",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
    ...
    assert commission_meta["source"] == FROM_WB_TARIFFS_COMMISSION_SOURCE
    assert commission_meta["status"] == "unavailable"
    assert commission_meta["reason"] == "invalid_json"
    assert commission_meta["account_id"] == commission_account.id
    assert commission_meta["fetched_rows"] == 0
    assert commission_meta["subjects_with_commission"] == 0
    assert commission_meta["commission_percent"] == {"main": None, "assorti": None}
...
def test_production_order_proposal_from_wb_reports_missing_report_commission_metadata(
    client,
    db_session,
    monkeypatch,
):
    ...
    commission_account = WbIntegrationAccount(
        name="WB Missing Report Commission",
        supplier_id=None,
        api_token="token-missing-report",
        is_active=True,
    )
    db_session.add(commission_account)
    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-COMMISSION-MISSING-REPORT",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
    ...
    assert commission_meta["source"] == FROM_WB_TARIFFS_COMMISSION_SOURCE
    assert commission_meta["status"] == "unavailable"
    assert commission_meta["reason"] == "missing_report"
    assert commission_meta["account_id"] == commission_account.id
    assert commission_meta["fetched_rows"] == 0
    assert commission_meta["subjects_with_commission"] == 0
    assert commission_meta["commission_percent"] == {"main": None, "assorti": None}
...
def test_production_order_proposal_from_wb_reports_invalid_payload_commission_metadata(
    client,
    db_session,
    monkeypatch,
):
    ...
    commission_account = WbIntegrationAccount(
        name="WB Invalid Payload Commission",
        supplier_id=None,
        api_token="token-invalid-payload",
        is_active=True,
    )
    db_session.add(commission_account)
    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-COMMISSION-INVALID-PAYLOAD",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
    ...
    def fake_httpx_get(url, *, headers=None, timeout=None):
        ...
        assert headers and "Authorization" in headers
        assert timeout is not None
        return FakeResponse(200, ["unexpected", "payload"])
    ...
    assert commission_meta["source"] == FROM_WB_TARIFFS_COMMISSION_SOURCE
    assert commission_meta["status"] == "unavailable"
    assert commission_meta["reason"] == "invalid_payload"
    assert commission_meta["account_id"] == commission_account.id
    assert commission_meta["fetched_rows"] == 0
    assert commission_meta["subjects_with_commission"] == 0
    assert commission_meta["commission_percent"] == {"main": None, "assorti": None}
...
def test_production_order_proposal_from_wb_reports_request_error_connect_error_commission_metadata(
    client,
    db_session,
    monkeypatch,
):
    ...
    commission_account = WbIntegrationAccount(
        name="WB Request Error Commission",
        supplier_id=None,
        api_token="token-request-error",
        is_active=True,
    )
    db_session.add(commission_account)
    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-COMMISSION-REQUEST-ERROR",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
    ...
    def fake_httpx_get(url, *, headers=None, timeout=None):
        ...
        assert headers and "Authorization" in headers
        assert timeout is not None
        request = planning_production_order_service.httpx.Request("GET", url)
        raise planning_production_order_service.httpx.ConnectError(
            "connection failed",
            request=request,
        )
    ...
    assert commission_meta["source"] == FROM_WB_TARIFFS_COMMISSION_SOURCE
    assert commission_meta["status"] == "unavailable"
    assert commission_meta["reason"] == "request_error:ConnectError"
    assert commission_meta["account_id"] == commission_account.id
    assert commission_meta["fetched_rows"] == 0
    assert commission_meta["subjects_with_commission"] == 0
    assert commission_meta["commission_percent"] == {"main": None, "assorti": None}
...
def test_production_order_proposal_from_wb_reports_http_500_commission_metadata(
    client,
    db_session,
    monkeypatch,
):
    ...
    commission_account = WbIntegrationAccount(
        name="WB HTTP 500 Commission",
        supplier_id=None,
        api_token="token-http-500",
        is_active=True,
    )
    db_session.add(commission_account)
    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-COMMISSION-HTTP-500",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
    ...
    assert commission_meta["source"] == FROM_WB_TARIFFS_COMMISSION_SOURCE
    assert commission_meta["status"] == "unavailable"
    assert commission_meta["reason"] == "wb_api_http_500"
    assert commission_meta["account_id"] == commission_account.id
    assert commission_meta["fetched_rows"] == 0
    assert commission_meta["subjects_with_commission"] == 0
    assert commission_meta["commission_percent"] == {"main": None, "assorti": None}
...
def test_production_order_proposal_from_wb_compact_explainability_mode(client, db_session):
    ...
    assert "method=allocation_decision_factors" in layer3_step
    assert "main:0|assorti:4|hold:0" in layer3_step
    assert "contract_status=ok" in layer3_step
    ...
    assert (
        "demand_units_until_arrival="
        f"{full_meta['arrival_projection']['demand_units_until_arrival']}"
    ) in arrival_projection_step
    assert (
        "projected_supply_units_before_arrival="
        f"{full_meta['arrival_projection']['projected_supply_units_before_arrival']}"
    ) in arrival_projection_step
    assert meta["reorder_policy"] == full_meta["reorder_policy"]
    assert meta["economic_buffer"] == full_meta["economic_buffer"]
    assert meta["in_flight_effective"] == full_meta["in_flight_effective"]
    ...
    assert f"method={full_meta['layer_2_allocation']['method_canonical']}" in layer2_step
    assert f"decision_gate={full_meta['layer_2_allocation']['decision_gate_canonical']}" in layer2_step
    ...
    assert f"contract_status={full_meta['layer_2_allocation']['contract']['status']}" in layer2_step
    ...
    assert meta["layer_2_allocation"]["objective_source"] == full_meta["layer_2_allocation"]["objective_source"]
    assert meta["layer_2_allocation"]["method"] == full_meta["layer_2_allocation"]["method"]
    ...
    assert meta["layer_2_allocation"]["legacy_alias_deprecation_plan"] == full_meta["layer_2_allocation"]["legacy_alias_deprecation_plan"]
    ...
    assert from_wb_meta["freshness_mode"] == full_meta["from_wb"]["freshness_mode"]
    assert from_wb_meta["requested_as_of_date"] == full_meta["from_wb"]["requested_as_of_date"]
    ...
    expected_runtime_daily_sales_by_bundle = {
        int(bundle_type_id): daily_sales
        for bundle_type_id, daily_sales in full_meta["from_wb"]["daily_sales_by_bundle"].items()
    }
    expected_runtime_wb_stock_by_bundle = {
...
def test_production_order_proposal_from_wb_uses_latest_sales_as_of_when_missing(client, db_session):
    ...
    expected_stock_age_days = (today_utc - stock_updated_at.date()).days
    body = response.json()
    wb_adapter_step = next(
        (step for step in body["explanation"]["steps"] if "WB ingestion adapter" in step),
        "",
...
def test_production_order_proposal_from_wb_without_stock_uses_latest_sales_as_of(client, db_session):
    seeded = _seed_article_bundle_base(db_session)
    today_utc = datetime.now(timezone.utc).date()
    sales_window_start = today_utc - timedelta(days=29)

    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-BT1-NOSTOCK",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
    ...
    assert "requested_as_of_date=none" in wb_adapter_step
    assert "freshness_mode=warn" in wb_adapter_step
    assert f"as_of_date={today_utc.isoformat()}" in wb_adapter_step
    assert "as_of_source=latest_sales" in wb_adapter_step
    assert f"sales_window={sales_window_start.isoformat()}..{today_utc.isoformat()}" in wb_adapter_step
    ...
    assert "freshness_status=missing_stock_data" in wb_adapter_step
    assert "freshness_sales_age_days=0" in wb_adapter_step
    ...
    assert from_wb_meta["freshness"]["status"] == "missing_stock_data"
...
def test_production_order_proposal_from_wb_without_sales_uses_none_as_of(client, db_session):
    ...
    assert from_wb_meta["wb_stock_updated_at_by_bundle"][bundle_key].startswith(
        stock_updated_at.strftime("%Y-%m-%dT%H:%M:%S")
    )
...
def test_production_order_proposal_from_wb_freshness_no_data_uses_admin_defaults_threshold_source(client, db_session):
    seeded = _seed_article_bundle_base(db_session)

    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-BT1-NODATA-ADMIN-DEFAULTS",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
...
def test_production_order_proposal_from_wb_warn_reports_stock_only_stale_metadata(client, db_session):
    seeded = _seed_article_bundle_base(db_session)
    today_utc = datetime.now(timezone.utc).date()
    sales_window_start = today_utc - timedelta(days=29)
    ...
    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-BT1-WARN-STOCK-STALE",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
    ...
    assert f"requested_as_of_date={today_utc.isoformat()}" in wb_adapter_step
    assert "freshness_mode=warn" in wb_adapter_step
    assert f"as_of_date={today_utc.isoformat()}" in wb_adapter_step
    assert "as_of_source=request" in wb_adapter_step
    assert f"sales_window={sales_window_start.isoformat()}..{today_utc.isoformat()}" in wb_adapter_step
...
def test_production_order_proposal_from_wb_warn_reports_sales_only_stale_metadata(client, db_session):
    seeded = _seed_article_bundle_base(db_session)
    ...
    today_utc = stock_updated_at.date()

    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-BT1-WARN-SALES-STALE",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
...
def test_production_order_proposal_from_wb_strict_rejects_no_data_with_admin_defaults_threshold_source(client, db_session):
    seeded = _seed_article_bundle_base(db_session)

    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-BT1-STRICT-NODATA-ADMIN-DEFAULTS",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
...
def test_production_order_proposal_from_wb_strict_rejects_missing_sales_with_stale_stock(client, db_session):
    seeded = _seed_article_bundle_base(db_session)
    today_utc = datetime.now(timezone.utc).date()
    ...
    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-BT1-STRICT-NOSALES-STALESTOCK",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
...
def test_production_order_proposal_from_wb_strict_request_stock_threshold_overrides_admin_defaults_for_missing_sales_with_stale_stock(client, db_session):
    seeded = _seed_article_bundle_base(db_session)
    today_utc = datetime.now(timezone.utc).date()
    ...
    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-BT1-STRICT-NOSALES-STALESTOCK-MIXED",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
...
def test_production_order_proposal_from_wb_strict_rejects_missing_stock_with_stale_sales(client, db_session):
    ...
    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-BT1-STRICT-NOSTOCK-STALESALES",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
...
def test_production_order_proposal_from_wb_strict_request_sales_threshold_overrides_admin_defaults_for_missing_stock_with_stale_sales(client, db_session):
    ...
    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-BT1-STRICT-NOSTOCK-STALESALES-MIXED",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
...
def test_production_order_proposal_from_wb_strict_request_thresholds_override_admin_defaults_to_fail(client, db_session):
    seeded = _seed_article_bundle_base(db_session)

    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-BT1-STRICT-REQUEST-OVERRIDE-ADMIN",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
...
def test_production_order_proposal_from_wb_strict_request_sales_threshold_overrides_admin_defaults_to_fail(client, db_session):
    seeded = _seed_article_bundle_base(db_session)

    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-BT1-STRICT-MIXED-SOURCE-SALES-FAIL",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
...
def test_production_order_proposal_from_wb_strict_request_stock_threshold_overrides_admin_defaults_to_fail(client, db_session):
    seeded = _seed_article_bundle_base(db_session)
    today_utc = datetime.now(timezone.utc).date()
    ...
    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-BT1-STRICT-MIXED-SOURCE-STOCK-FAIL",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
...
def test_production_order_proposal_from_wb_uses_mirrored_mixed_freshness_threshold_sources(client, db_session):
    seeded = _seed_article_bundle_base(db_session)

    db_session.add(
        ArticleWbMapping(
            article_id=seeded["article"].id,
            wb_sku="WB-PO-BT1-MIXED-THRESHOLDS-2",
            bundle_type_id=seeded["bundle_type"].id,
            size_id=seeded["size_s"].id,
        )
    )
