...
- [x] Layer 2 penalty-weight calibration evidence is documented with flip boundaries and frozen defaults, regressions assert emitted `objective_parameters` + `objective_source` in explainability payloads, and the documented stockout-profile boundary is helper-regression locked (`0.84 -> main`, `0.85 -> assorti`).
