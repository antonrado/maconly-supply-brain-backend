...
- Penalty-weight calibration evidence is now mandatory and documented: freeze Layer 2 defaults (`capital_cost_rate=0.08`, `stockout_penalty_weight=1.0`, `overstock_penalty_weight=1.0`), track approximate flip boundaries in the casebook, assert emitted `objective_parameters` + `objective_source` in regression payloads, and lock the documented stockout-profile boundary with explicit below/above-threshold helper regression values.
