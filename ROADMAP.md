...
- Multi-regime release evidence is now explicit and regression-locked across both direct and `from-wb` proposal paths, and the same evidence now also survives compact-mode projection: `stockout_dominates`, `overstock_dominates`, and `commission_price_conflict` e2e cases must prove objective-vs-profit disagreement and expected Layer 5 signal behavior without adapter drift or compaction drift.
- Penalty-weight calibration evidence is now mandatory and documented: freeze Layer 2 defaults (`capital_cost_rate=0.08`, `stockout_penalty_weight=1.0`, `overstock_penalty_weight=1.0`), track approximate flip boundaries in the casebook, assert emitted `objective_parameters` + `objective_source` in regression payloads, and lock the documented stockout/overstock/commission-conflict profile boundaries with explicit below/above-threshold helper regression values.
- Layer 2 objective coefficients now also have explicit precedence/source-tracing evidence on both direct and `from-wb` production-order paths: `layer2_capital_cost_rate`, `layer2_stockout_penalty_weight`, and `layer2_overstock_penalty_weight` are regression-locked for `global_default`, `admin_defaults`, and `request` tiers via both `layer_2_allocation` and `alpha_proxy_economics.layer_proxy_source` / `layer_2_objective_parameters`.
