...
- Multi-regime release evidence is now explicit and regression-locked across both direct and `from-wb` proposal paths: `stockout_dominates`, `overstock_dominates`, and `commission_price_conflict` e2e cases must prove objective-vs-profit disagreement and expected Layer 5 signal behavior without adapter drift.
