...
- Planning Core production-order proposal endpoint added:
  ...
  - `from-wb` explanation now includes requested/effective `as_of_date` source trace, resolved sales window bounds, and adapter snapshots for `daily_sales_by_bundle` + `wb_stock_by_bundle` to make WB-derived input reconstruction auditable.
