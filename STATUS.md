...
- Capital-aware line ranking/constraint evidence is now helper-regression locked: objective-per-capital ranking can deterministically prioritize lower gross-profit lines when penalties dominate, and budget-limited allocation follows this ranking rather than silent profit-only ordering.
...
- Added helper-level economic evidence regressions for objective-per-capital ranking and budget-constrained allocation ordering, plus Layer 4 violated-case regression for scenario delta baseline inconsistency.
