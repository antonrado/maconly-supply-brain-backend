...
- **Multi-instance scheduler**
  - Текущая реализация планировщика запускается в контексте процесса FastAPI-приложения.
  - При запуске нескольких экземпляров backend-а (несколько контейнеров/процессов) каждая копия будет создавать свои записи в `monitoring_snapshots`.
  - Требуется явно определить стратегию: один «лидер»-инстанс, внешний оркестратор задач или доп. защита на уровне БД.
